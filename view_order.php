...
if(isset($_GET["pdf"]) && isset($_GET['order_id']))
{
	...
	$output = '';
	$query = "SELECT * FROM inventory_order WHERE inventory_order_id = '$_GET[order_id]' LIMIT 1";
	...
	while($row = mysqli_fetch_assoc($statement))
	{
		...
		$query = "SELECT * FROM inventory_order_product WHERE inventory_order_id = '$_GET[order_id]'";
		$statement = mysqli_query($connect, $query);
		$count = 0;
		$total = 0;
